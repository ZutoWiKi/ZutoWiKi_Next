...
interface Props {
  ...
  id: number;
  size?: number;
  showProfile?: boolean; // 프로필을 표시할지 여부
  fallbackComponent?: React.ReactNode; // 대체 컴포넌트
}
...
export default function UserProfileColorImage({
  ...
  colorClass = id == 1 ? "bg-black" : colorClass;

  // id를 기반으로 랜덤 이미지 선택 (같은 id는 항상 같은 이미지)
  const srcIdx = id % SRC.length;
  const src = SRC[srcIdx];
